...
export default function Customers() {
  ...
  const fetchCustomers = useCallback(async () => {
    ...
    try { setCustomers(await api.customers.list()); }
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    try {
      await api.customers.update(editCustomer.id, editForm);
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1 sm:flex-none">
              ...
              <input
                ...
                className="pl-8 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-44"
              />
            </div>
            ...
            <button onClick={() => setShowAdd(v => !v)} title="Add new customer"
        ...
        {/* Add form */}
        {showAdd && (
          <form onSubmit={handleAdd} className="m-4 sm:m-5 p-4 bg-blue-50 rounded-xl border border-blue-100 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => { setShowAdd(false); setFormErr(''); }}
                className="p-1 text-slate-400 hover:text-slate-600 rounded-lg hover:bg-blue-100 transition-colors">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {[
                { field: 'name',    label: 'Full Name',      placeholder: 'Ahmed Transport Co.',  required: true },
                { field: 'phone',   label: 'Phone Number',   placeholder: '+92-300-1234567' },
                { field: 'email',   label: 'Email Address',  placeholder: 'contact@company.pk' },
                { field: 'address', label: 'City / Address', placeholder: 'Lahore, Punjab' },
            ...
            <div className="flex gap-2 justify-end pt-1">
              ...
              <button type="submit" disabled={saving}
                className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-60">
                {saving ? <><Loader2 size={13} className="animate-spin" />Saving...</> : 'Save Customer'}
              </button>
            </div>
          </form>
        )}
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-2.5 space-y-1">
                    {c.phone   && <div className="flex items-center gap-2 text-xs text-slate-500"><Phone size={10} className="flex-shrink-0 text-slate-400" /><span className="truncate">{c.phone}</span></div>}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              ...
              <button onClick={() => setEditCustomer(null)} className="p-2 hover:bg-slate-100 rounded-lg transition-colors">
